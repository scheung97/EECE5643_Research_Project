...
pythonData = readtable('covid_data.csv', 'PreserveVariableNames', true);

% latestWebscrapeDate = pythonData{size(pythonData,1), 1};
% latestWebscrapeTime = pythonData{size(pythonData,1), 2};
% latestTotCases = pythonData{size(pythonData,1), 3};
% latestTotAdministeredVacs = pythonData{size(pythonData,1), 4};
% latestTotDeaths = pythonData{size(pythonData,1), 5};
% latestInfectRate = pythonData{size(pythonData,1), 6};
latestDeathRate = pythonData{size(pythonData,1), 7};
...
Mortality_Rate      = latestDeathRate; %0.026;                                    % Likelihood of death if infected.
